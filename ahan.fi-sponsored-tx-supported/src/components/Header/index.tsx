...
const Header: React.FC<HeaderProps> = ({ connection, accountAddress }) => {
  const [isCopied, setCopied] = useClipboard(accountAddress ?? "", { successDuration: 1000 });
          ...
          <div className="wrapper">
            ...
            {connection && accountAddress && (
              <div className="button-group">
                <Button>{stringTrimmer(accountAddress, "...", 4)}</Button>
                <button className="account-buttons" onClick={setCopied}>
                  {isCopied ? (
                    <FaCopy size={16} color="#222" />
                  ) : (
                    <FaRegCopy size={16} color="#222" />
                  )}
                </button>

                <button className="account-buttons" onClick={() => window.location.reload()}>
                  <IoLogOutSharp size={20} color="#222" />
                </button>
              </div>
            )}
          </div>
